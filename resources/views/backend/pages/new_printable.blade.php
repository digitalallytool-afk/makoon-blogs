...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Add New Printable</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <form action="{{ route('printables.store') }}" method="POST" enctype="multipart/form-data" id="printable-form">
                @csrf
                {{-- Hidden status field controlled by the publish/draft buttons --}}
                <input type="hidden" name="status" id="printable-status" value="published">

                {{-- Hidden fields populated after AJAX file upload completes --}}
                <input type="hidden" name="file_path" id="file_path" value="{{ old('file_path') }}">
                    ...
                    <div class="col-12 col-xl-8">
                            ...
                            <div class="card-body p-4">
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control form-control-lg border-0 bg-light-subtle shadow-none font-20"
                                           id="printable-name" name="name" value="{{ old('name') }}"
                                ...
                                <div class="mb-0">
                                    ...
                                    <textarea class="form-control border-0 bg-light" id="printable-desc" name="description" rows="5"
                                              placeholder="Provide details about what this printable contains...">{{ old('description') }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card radius-10 border-0 shadow-sm">
                            <div class="card-header bg-transparent border-bottom pt-3 pb-3">
                                ...
                                <small class="text-muted font-11 d-block mt-1">Upload PDF, ZIP, or doc up to 50 MB.</small>
                            </div>
                            <div class="card-body p-4">
                                <div class="mb-3">
                                    <label for="file-upload-input" class="form-label font-weight-bold text-secondary font-12 text-uppercase">Select File</label>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('file-upload-input').addEventListener('change', function(e) {
                ...
                xhr.addEventListener('load', function() {
                    ...
                    if (xhr.status === 200) {
                        ...
                        statusText.textContent = 'Upload complete! File successfully processed.';
            ...
            document.getElementById('printable-form').addEventListener('submit', function(e) {
                ...
                if (!filePath) {
                    e.preventDefault();
                    alert('Please select and upload a printable file first.');
                }
            });
        </script>
    @endpush
