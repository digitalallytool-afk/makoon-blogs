...
@push('styles')
    <style>
        ...
        .story-content table tr:nth-of-type(even) {
        ...
        <div class="page-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-4">
                ...
                <div class="ms-auto d-flex gap-2">
                    <a href="{{ route('allStory') }}" class="btn btn-outline-secondary radius-30 px-3">
                        <i class="bx bx-arrow-back me-1"></i>Back to List
                    </a>
                    @can('edit-stories')
                        <a href="{{ route('stories.edit', $story->id) }}" class="btn btn-warning text-dark radius-30 px-3">
                            <i class="bx bx-edit me-1"></i>Edit Story
                        </a>
                    @endcan
                </div>
            </div>
                    ...
                    <div class="card radius-10 border-0 shadow-sm overflow-hidden mb-4">
                        @if($story->featured_image)
                            <div class="position-relative" style="max-height: 400px; overflow: hidden; background-color: #f8f9fa;">
                                <img src="{{ asset($story->featured_image) }}" class="img-fluid w-100" style="object-fit: cover; max-height: 400px;" alt="{{ $story->title }}">
                                <div class="position-absolute bottom-0 start-0 w-100 p-4 bg-gradient-dark text-white" style="background: linear-gradient(transparent, rgba(0,0,0,0.85));">
                                    <span class="badge bg-primary px-3 py-1 radius-30 mb-2">{{ $story->storyCategory->name ?? 'Uncategorized' }}</span>
                                    <h2 class="mb-0 font-weight-bold text-white fs-3 shadow-text">{{ $story->title }}</h2>
                                </div>
                            </div>
                        @else
                            <div class="card-body border-bottom p-4">
                                <span class="badge bg-primary px-3 py-1 radius-30 mb-2">{{ $story->storyCategory->name ?? 'Uncategorized' }}</span>
                                <h2 class="mb-0 font-weight-bold text-dark fs-2">{{ $story->title }}</h2>
                            </div>
                        @endif
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="row g-3 align-items-center border-bottom pb-3 mb-4 text-secondary font-13">
                                <div class="col-6 col-md-3">
                                    ...
                                    <div class="d-flex align-items-center gap-2 mt-1">
                                        @if($story->author && $story->author->image)
                                            <img src="{{ asset($story->author->image) }}" class="rounded-circle border" width="28" height="28" style="object-fit: cover;" alt="{{ $story->author->name }}">
                                        @endif
                                        <span class="font-weight-bold text-dark">{{ $story->author->name ?? '—' }}</span>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    ...
                                    <div class="d-flex align-items-center gap-1 mt-1 text-dark">
                                        ...
                                        <span>{{ $story->created_at->format('M d, Y H:i') }}</span>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                ...
                                <div class="col-6 col-md-3">
                                    ...
                                    <div class="d-flex align-items-center gap-1 mt-1 text-dark">
                                        ...
                                        <span class="font-weight-bold">{{ number_format($story->view_count) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if($story->excerpt)
                                <div class="p-4 bg-light rounded border-start border-4 border-primary mb-4">
                                    ...
                                    <p class="mb-0 text-dark font-15 italic" style="font-style: italic;">"{{ $story->excerpt }}"</p>
                                </div>
                            @endif

                            <!-- Content -->
                            <div class="story-content text-dark font-16 lh-lg">
                                {!! $story->content !!}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card radius-10 border-0 shadow-sm mb-0">
                        ...
                        <div class="card-body p-4">
                            <div class="mb-3">
                                ...
                                <p class="mb-0 font-weight-bold text-dark font-14">{{ $story->meta_title ?? '—' }}</p>
                            </div>
                            <div class="mb-3">
                                <span class="d-block font-11 text-uppercase text-muted mb-1">Meta Description</span>
                                <p class="mb-0 text-secondary font-13">{{ $story->meta_description ?? '—' }}</p>
                            </div>
                            <div class="mb-3">
                                <span class="d-block font-11 text-uppercase text-muted mb-1">Meta Keywords</span>
                                <p class="mb-0 text-secondary font-13">
                                    @if($story->meta_keywords)
                                        @foreach(explode(',', $story->meta_keywords) as $keyword)
                    ...
                    <div class="card radius-10 border-0 shadow-sm mb-0">
                        ...
                        <div class="card-body p-4">
                            <div class="mb-3">
                                ...
                                <a href="{{ $story->canonical_url }}" target="_blank" class="text-primary text-break font-12">
                                    {{ $story->canonical_url ?? '—' }}
                            ...
                            <div class="mb-3">
                                ...
                                <code class="font-12">{{ $story->slug }}</code>
                            </div>
                            <div class="mb-0">
                                <span class="d-block font-11 text-uppercase text-muted mb-1">Story Category</span>
