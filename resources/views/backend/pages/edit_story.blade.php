...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <form action="{{ route('stories.update', $story->id) }}" method="POST" enctype="multipart/form-data" id="story-form">
                @csrf
                @method('PUT')
                {{-- Hidden status field controlled by the publish/draft buttons --}}
                <input type="hidden" name="status" id="story-status" value="{{ $story->status }}">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12 col-xl-8">
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-4">
                            <div class="card-body p-4">
                                <div class="mb-4">
                                    <label for="story-title" class="form-label font-weight-bold text-secondary font-12 text-uppercase">Story Title</label>
                                    <input type="text" class="form-control form-control-lg border-0 bg-light-subtle shadow-none font-20"
                                           id="story-title" name="title" value="{{ old('title', $story->title) }}"
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control border-0 bg-light-subtle shadow-none font-14"
                                           id="story-slug" name="slug" value="{{ old('slug', $story->slug) }}"
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control" id="story-content" name="content" required>{{ old('content', $story->content) }}</textarea>
                                </div>

                                <!-- Excerpt -->
                                <div class="mb-0">
                                    <label for="story-excerpt" class="form-label font-weight-bold text-secondary font-12 text-uppercase">Excerpt</label>
                                    <textarea class="form-control border-0 bg-light" id="story-excerpt" name="excerpt" rows="3"
                                              placeholder="Write a brief story summary...">{{ old('excerpt', $story->excerpt) }}</textarea>
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            <div class="card-header bg-transparent border-bottom pt-3 pb-3">
                                ...
                                <small class="text-muted font-11 d-block mt-1">Improve search engine visibility. Leave blank to use story title &amp; excerpt.</small>
                            </div>
                            <div class="card-body p-4">
                                <div class="row g-3">
                                    <!-- Meta Title -->
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea class="form-control border-0 bg-light"
                                                  ...
                                                  maxlength="500">{{ old('meta_description', $story->meta_description) }}</textarea>
                                        <div class="d-flex justify-content-between mt-1">
                    ...
                    <div class="col-12 col-xl-4 d-flex flex-column gap-4">
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0" id="seo-analyzer-card">
                            ...
                            <div class="card-body pt-2">
                                ...
                                <div class="d-flex align-items-center gap-3 mb-3 p-2 rounded" style="background:#f8f9fa;">
                                    ...
                                    <div>
                                        <div class="font-weight-bold font-13" id="seo-score-label">Analysing...</div>
                                        <div class="text-muted font-11" id="seo-score-sub">SEO score loading from saved fields</div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div style="border:1px solid #e0e0e0;border-radius:8px;padding:10px 12px;background:#fff;">
                                        <div id="serp-url" style="font-size:11px;color:#1a7f37;margin-bottom:2px;word-break:break-all;">makoons.com › stories › {{ $story->slug }}</div>
                                        <div id="serp-title" style="font-size:14px;color:#1a0dab;font-weight:600;line-height:1.3;margin-bottom:3px;">{{ $story->meta_title ?: $story->title }}</div>
                                        <div id="serp-desc" style="font-size:12px;color:#4d5156;line-height:1.5;">{{ $story->meta_description ?: 'Your meta description will appear here.' }}</div>
                                    </div>
                                </div>
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-footer bg-light-subtle p-3 border-top-0">
                                <div class="d-flex flex-wrap gap-1 align-items-center">
                                    <a href="{{ route('allStory') }}" class="btn btn-outline-secondary px-2 py-1" style="font-size: 11px; border-radius: 20px; font-weight: 500;">
                                        Cancel
                                    </a>
                                    <!-- Save as Draft -->
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-body pt-1">
                                <label for="story-category" class="form-label font-weight-bold text-secondary font-12 text-uppercase mb-2">Select Category</label>
                                <select class="form-select" id="story-category" name="story_category_id" required>
                                    <option value="" disabled>-- Choose Category --</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('story_category_id', $story->story_category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <small class="text-muted d-block mt-2 font-11">Classify this story under a category.</small>
                            </div>
                        </div>
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-body pt-1">
                                <label for="story-author" class="form-label font-weight-bold text-secondary font-12 text-uppercase mb-2">Assign Author</label>
                                <select class="form-select" id="story-author" name="author_id" required>
                                    <option value="" disabled>-- Choose Author --</option>
                                    @foreach($authors as $author)
                                        <option value="{{ $author->id }}" {{ old('author_id', $story->author_id) == $author->id ? 'selected' : '' }}>{{ $author->name }}</option>
                                    @endforeach
                                </select>
                                <small class="text-muted d-block mt-2 font-11">Assign an author profile to this story.</small>
                            </div>
                        </div>
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-body pt-1">
                                <input type="hidden" name="featured_image_url" id="featured_image_url" value="{{ $story->featured_image }}">
                                <div class="d-flex flex-column gap-2">
                                    ...
                                    <input type="file" class="form-control" id="story-featured-image" name="featured_image" accept="image/*" onchange="clearSelectedFeaturedImage()">
                                </div>
                                <div class="mt-3 {{ $story->featured_image ? '' : 'd-none' }}" id="featured-image-preview-container">
                                    <img src="{{ $story->featured_image ? asset($story->featured_image) : '' }}" id="featured-image-preview" class="img-fluid rounded border shadow-sm" style="max-height: 150px; object-fit: cover; width: 100%;">
        ...
        <script>
            ...
            (function() {
                ...
                function analyse() {
                    ...
                    const checks = [
                        { label: 'Meta Title length', run: () => { if(!mTitle) return 'fail'; const l=mTitle.length; if(l>=50&&l<=60) return 'pass'; if(l>=40&&l<=70) return 'warn'; return 'fail'; }, msgs: { pass:`Meta Title: ${mTitle.length} chars (ideal 50–60)`, warn:`Meta Title: ${mTitle.length} chars (try for 50–60)`, fail:!mTitle?'Meta Title is empty':`Meta Title: ${mTitle.length} chars (too short/long)` } },
                        { label: 'Focus keyword in Title', run: () => { if(!focusKw||!mTitle) return 'fail'; return mTitle.toLowerCase().includes(focusKw.toLowerCase())?'pass':'fail'; }, msgs: { pass:`"${focusKw}" found in meta title`, warn:'', fail:!focusKw?'Add at least one keyword':`"${focusKw}" missing from meta title` } },
                        { label: 'Power words in Title', run: () => { if(!mTitle&&!title) return 'fail'; return hasPowerWord(mTitle||title)?'pass':'warn'; }, msgs: { pass:'Title contains a power word 🎯', warn:'Add power words like Best, Guide, Tips…', fail:'Title is empty' } },
                        { label: 'Meta Description length', run: () => { if(!mDesc) return 'fail'; const l=mDesc.length; if(l>=150&&l<=160) return 'pass'; if(l>=120&&l<=180) return 'warn'; return 'fail'; }, msgs: { pass:`Description: ${mDesc.length} chars (perfect!)`, warn:`Description: ${mDesc.length} chars (aim for 150–160)`, fail:!mDesc?'Meta Description is empty':`Description: ${mDesc.length} chars (too short/long)` } },
                        { label: 'Focus keyword in Description', run: () => { if(!focusKw||!mDesc) return 'fail'; return mDesc.toLowerCase().includes(focusKw.toLowerCase())?'pass':'fail'; }, msgs: { pass:`"${focusKw}" found in description`, warn:'', fail:!focusKw?'Set a focus keyword first':`"${focusKw}" missing from description` } },
                        { label: 'Story Title length', run: () => { if(!title) return 'fail'; if(title.length<=60) return 'pass'; if(title.length<=70) return 'warn'; return 'fail'; }, msgs: { pass:`Story title length OK (${title.length} chars)`, warn:`Story title a bit long (${title.length} chars)`, fail:!title?'Story title is empty':`Story title too long (${title.length} chars)` } },
                        { label: 'Keywords filled', run: () => kwList.length>0?'pass':'fail', msgs: { pass:`${kwList.length} keyword(s) added`, warn:'', fail:'Add comma-separated keywords' } },
                        { label: 'Keyword count (no stuffing)', run: () => { if(kwList.length===0) return 'fail'; if(kwList.length<=5) return 'pass'; if(kwList.length<=8) return 'warn'; return 'fail'; }, msgs: { pass:`${kwList.length} keywords (ideal ≤5)`, warn:`${kwList.length} keywords (consider reducing)`, fail:kwList.length===0?'No keywords set':`${kwList.length} keywords (too many!)` } },
                        { label: 'Keyword density', run: () => { if(!focusKw||!content) return 'warn'; if(density<=3) return 'pass'; if(density<=5) return 'warn'; return 'fail'; }, msgs: { pass:`Keyword density: ${density.toFixed(1)}% (good)`, warn:!content?'Add content to check density':`Keyword density: ${density.toFixed(1)}% (slightly high)`, fail:`Keyword density: ${density.toFixed(1)}% (too high!)` } },
                        { label: 'Content word count', run: () => { if(wc>=300) return 'pass'; if(wc>=100) return 'warn'; return 'fail'; }, msgs: { pass:`${wc} words (great length!)`, warn:`${wc} words (aim for 300+)`, fail:`${wc} words (too short for Google)` } },
                    ];

                    let score = 0;
                    const icons = { pass:'<span style="color:#198754;font-size:13px;">✔</span>', warn:'<span style="color:#ffc107;font-size:13px;">⚠</span>', fail:'<span style="color:#dc3545;font-size:13px;">✖</span>' };
                    const points = { pass:10, warn:5, fail:0 };

                    checksList.innerHTML = checks.map(c => {
                        const status = c.run(); score += points[status];
                    ...
                    if (pct>=70) { gauge.setAttribute('stroke','#198754'); scoreBadge.style.cssText='background:#d1e7dd;color:#0a3622;'; scoreLabel.textContent='Good SEO Score 🟢'; scoreSub.textContent='Your SEO looks solid. Keep improving!'; }
                    else if (pct>=40) { gauge.setAttribute('stroke','#ffc107'); scoreBadge.style.cssText='background:#fff3cd;color:#664d03;'; scoreLabel.textContent='Average Score 🟡'; scoreSub.textContent='Some improvements needed for better ranking.'; }
                    else { gauge.setAttribute('stroke','#dc3545'); scoreBadge.style.cssText='background:#f8d7da;color:#58151c;'; scoreLabel.textContent='Poor SEO Score 🔴'; scoreSub.textContent='Fill in meta fields and keywords to improve.'; }
                }

                [titleEl, metaTitle, metaDesc, metaKw].forEach(el => { if(el) el.addEventListener('input', analyse); });
                setInterval(() => {
                    if (window.tinymce && tinymce.activeEditor) {
                        const val = tinymce.activeEditor.getContent();
                        if (contentEl && contentEl.value !== val) { contentEl.value = val; analyse(); }
                    }
                }, 2000);
                analyse();
            })();
