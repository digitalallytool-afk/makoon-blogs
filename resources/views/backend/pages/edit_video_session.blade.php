...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <form action="{{ route('videoSessions.update', $videoSession->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="status" id="video-status" value="{{ $videoSession->status }}">
                            ...
                            <div class="card-body p-4">
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control form-control-lg border-0 bg-light-subtle shadow-none font-20"
                                           id="session-title" name="title" value="{{ old('title', $videoSession->title) }}"
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control border-0 bg-light-subtle shadow-none font-14"
                                           id="session-slug" name="slug" value="{{ old('slug', $videoSession->slug) }}"
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-select border-0 bg-light" id="session_category_id" name="session_category_id" required style="padding: 10px 15px;">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('session_category_id', $videoSession->session_category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-0">
                                    ...
                                    <textarea class="form-control border-0 bg-light" id="session-desc" name="description" rows="5"
                                              placeholder="Provide details about this video session...">{{ old('description', $videoSession->description) }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body p-4">
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control border-0 bg-light" id="video_url" name="video_url"
                                           value="{{ old('video_url', $videoSession->video_url) }}" placeholder="https://www.youtube.com/watch?v=..." required
                                           style="padding: 10px 15px;">
                                </div>
                    ...
                    <div class="col-12 col-xl-4 d-flex flex-column gap-4">
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-body pt-1">
                                <!-- Status indicator -->
                                <div class="d-flex align-items-center gap-2 mb-3 py-2 px-3 bg-light rounded font-13 text-capitalize">
                                    <i class="bx bx-info-circle text-secondary fs-5"></i>
                                    <span class="text-secondary">Status: </span>
                                    <span class="font-weight-bold text-dark text-capitalize" id="status-label">{{ $videoSession->status }}</span>
                                </div>
                            </div>
                        ...
                        <div class="card radius-10 border-0 shadow-sm mb-0">
                            ...
                            <div class="card-body pt-1">
                                <input type="hidden" name="featured_image_url" id="featured_image_url" value="{{ $videoSession->image }}">
                                ...
                                <div class="mt-3 {{ $videoSession->image ? '' : 'd-none' }}" id="featured-image-preview-container">
                                    <img src="{{ $videoSession->image ? asset($videoSession->image) : '' }}" id="featured-image-preview" class="img-fluid rounded border shadow-sm" style="max-height: 150px; object-fit: cover; width: 100%;">
